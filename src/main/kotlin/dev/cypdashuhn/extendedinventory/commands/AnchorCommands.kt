...
fun <T : CanSuggest> T.suggestAnchorNames(): T = suggestStrings {
    val player = playerOrNull ?: return@suggestStrings emptyList()
    val profileId = HotbarManager.getState(player).profileId ?: return@suggestStrings emptyList()
    transaction {
        AnchorManager.allForProfile(profileId).map { it.name }
    }
}

fun ChildrenScope.anchors() = literal("anchor") {
    literal("add") {
        string("name") {
            integer("x") {
                integer("y").onExecute {
                    ...
                    player.msg("${T.green}Anchor '${T.white}$name${T.green}' added at ($x, $y).")
                }
            }
        }.matches(RESOURCE_NAME_REGEX) { raw -> player.msg("${T.red}Invalid anchor name '${T.white}$raw${T.red}'.") }
    }
    literal("delete") {
        ...
        string("oldName") {
            string("newName")
                ...
                .onExecute {
                    ...
                    val profileId = HotbarManager.ensureProfile(player)
                    val success = AnchorActions.renameAnchor(player, profileId, oldName, newName)
                    if (success) {
                        player.msg("${T.green}Anchor renamed to '${T.white}$newName${T.green}'.")
                    } else {
    ...
    literal("info") {
        string("name").suggestAnchorNames().onExecute {
            ...
            if (anchor != null) {
                player.msg("${T.green}Anchor '${T.white}${anchor.name}${T.green}': position (${anchor.x}, ${anchor.y})")
            } else {
